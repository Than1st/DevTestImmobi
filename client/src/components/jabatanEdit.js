...
import { GetJabatan, PutJabatan } from "../actions/JabatanAction";
import { useForm } from "react-hook-form";
import { PostJabatan } from "../actions/JabatanAction";
import Swal from "sweetalert2";
import { useNavigate, useParams } from "react-router-dom";

export const JabatanEdit = () => {
  ...
  const putJabatan = (data) => {
    const dataJson = {
      nama_jabatan: data.nama_jabatan,
      id_department: selectedDepartment,
    };
    dispatch(PutJabatan(dataJson, params.id));
    setIsPost(true);
  };
  useEffect(() => {
    if (isPost) {
    ...
    var defaultValue = {};
    defaultValue.nama_jabatan = sessionStorage.getItem("nama_jabatan");
    reset({ ...defaultValue });
  }, [isPost]);
  ...
  return (
    <div className="container">
      ...
      <form key={1} onSubmit={handleSubmit(putJabatan)}>
        <div class="mb-3">
